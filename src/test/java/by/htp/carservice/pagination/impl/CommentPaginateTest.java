...
public class CommentPaginateTest {
    ...
    public void testPaginate() throws ServiceException {
        List<Comment> carList = factory.getCommentPagination().paginate(paginate);
        assertEquals(carList.size(), 0);
    }

    @Test
    public void testPaginateById() throws ServiceException {
        List<Comment> carList = factory.getCommentPagination().paginateById(paginate,1);
        assertEquals(carList.size(), 0);
    }
}