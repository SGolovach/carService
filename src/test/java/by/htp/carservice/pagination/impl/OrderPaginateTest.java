...
public class OrderPaginateTest {
    ...
    public void testPaginate() throws ServiceException {
        List<Order> carList = factory.getOrderPagination().paginate(paginate);
        assertEquals(carList.size(), 0);
    }

    @Test
    public void testPaginateById() throws ServiceException {
        List<Order> carList = factory.getOrderPagination().paginateById(paginate,1);
        assertEquals(carList.size(), 0);
    }
}