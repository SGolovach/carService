...
public class CarPaginateTest {
    ...
    public void testPaginate() throws ServiceException {
        List<Car> carList = factory.getCarPagination().paginate(paginate);
        assertEquals(carList.size(), 0);
    }

    @Test
    public void testPaginateById() throws ServiceException {
        List<Car> carList = factory.getCarPagination().paginateById(paginate,1);
        assertEquals(carList.size(), 0);
    }
}